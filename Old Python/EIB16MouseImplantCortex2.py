...
for i, (entry, exit) in enumerate(ground_holes):
    ...
    depth_guide_scad1 += f"""
    translate([{exit[0]}, {exit[1]}, {exit[2]:.4f}])
        cylinder(h = {height_depth_g:.4f}, r = {outer_radius_g}, $fn=60);
"""
...
for i, (entry, exit) in enumerate(ground_holes):
    ...
    depth_guide_scad2 += f"""
    translate([{exit[0]}, {exit[1]}, {exit[2]:.4f}])
        cylinder(h = {height_depth_g:.4f}, r = {inner_radius_g}, $fn=60);
"""
